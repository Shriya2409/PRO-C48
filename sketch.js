...
function preload(){
    p1TrophyImage=loadImage("Images/trophy.png");
}
...
function setup(){
    ...
    g5Teddy =new Gift5(1000,100,55,55);
        ...
        g2Toy =new Gift7(600,100,55,55);
        g3Train =new Gift8(1000,100,55,55);
    ...
    p1Trophy =createSprite(600,height/2,100,100);
    p1Trophy.addImage(p1TrophyImage)
...
function draw(){
   ...
   ring3.display();
    ...
    if(score>50 && State==="Play"){
    gamestate=LEV2;
    score=0;
    State="start";
    /*World.remove(world,g1Dress);
    World.remove(world,g2Ball);
    World.remove(world,g3Aeroplane);
    World.remove(world,g4Robot);
    World.remove(world,g5Teddy);*/
}

if (score<50 && State==="End"){
    textSize(45);
text("You Lost",300,height/2);
}

if (ring2.body.position.x<0 ||  ring2.body.position.x>width){
    if (ring3.body.position.x<0 || ring3.body.position.x>1200){
        if (ring4.body.position.x<0 || ring4.body.position.x>1200){
State="End";
}}}
 
if (score>50 && gamestate===LEV2){
    drawSprites();
    fill("yellow");
    textSize(45);
    text("Congratulations!!! You Have Won",300,height/2);
}

if (score<50 && State==="GameOver"){
    fill("red");
    textSize(45);
    text("GameOver",300,height/2);
    }

if (ring4.body.position.x<0 ||  ring4.body.position.x>width){
   if (ring5.body.position.x<0 || ring5.body.position.x>1200){
        if (ring6.body.position.x<0 || ring6.body.position.x>1200){
State="GameOver";
}}}
